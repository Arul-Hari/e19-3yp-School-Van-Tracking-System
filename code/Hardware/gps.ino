...
void publishData() {
  ...
  double currentLatitude = coordinates[currentCoordinateIndex][0];
  double currentLongitude = coordinates[currentCoordinateIndex][1];
  ...
  localDoc["latitude"] = currentLatitude;
  localDoc["longitude"] = currentLongitude;
  localDoc["speed"] = 0.0;  // You can set a fixed speed or omit it if not needed
  ...
  // Move to the next set of coordinates in the array
  currentCoordinateIndex = (currentCoordinateIndex + 1) % (sizeof(coordinates) / sizeof(coordinates[0]));
  delay(5000); // Adjust the delay as needed between coordinates (in milliseconds)
}
